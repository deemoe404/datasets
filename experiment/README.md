...
- [world_model_baselines_v1](./families/world_model_baselines_v1/README.md): `kelmarsh` only, farm-synchronous, repo-local controlled baselines: persistence plus TFT-style, TimeXer-style, DGCRN-style, Chronos-2 zero-shot official, iTransformer-style, and MTGNN calendar-graph task-adapted comparisons (`world_model_v1`)
- [world_model_hardened_baselines_v1](./families/world_model_hardened_baselines_v1/README.md): `kelmarsh` only, farm-synchronous, phase-1 adapter sanity track with official provenance annotations but repo-local backend execution (`world_model_v1`)
- [world_model_official_baselines_v2](./families/world_model_official_baselines_v2/README.md): `kelmarsh` only, farm-synchronous, paper-grade official/official-core baseline track with hard source guards, feature budgets, validation-only selection manifests, and adapter-level provenance (`world_model_v1`)
