...
- [agcrn](./families/agcrn/README.md): Kelmarsh-only, farm-synchronous, 6-protocol AGCRN pilot (`power_only`, `power_ws_hist`, `power_wd_hist_sincos`, `power_wd_yaw_hist_sincos`, `power_wd_yaw_lrpm_hist_sincos`, `power_ws_wd_hist_sincos`)
...
- The active dataset-side protocol surface is `power_only` + `power_ws_hist` + `power_wd_hist_sincos` + `power_wd_yaw_hist_sincos` + `power_wd_yaw_lrpm_hist_sincos` + `power_ws_wd_hist_sincos`, and their semantics live exclusively in `src/wind_datasets/feature_protocols.py`.
