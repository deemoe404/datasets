...
- `agcrn_official_aligned` remains the baseline AGCRN family; `agcrn_masked` validates masked task bundles; `world_model_agcrn_v1` validates the full `world_model_v1` protocol with `series` + `known_future` + `static` + `pairwise`; `world_model_rollout_v1` validates the deterministic filtering-plus-rollout world-model implementation on the same bundle contract.
- All current families target `kelmarsh` + `penmanshiel`; `sdwpf_kddcup` is excluded because the strict `24h -> 6h` task has zero usable windows, and `hill_of_towie` is excluded because the masked and world-model protocols are not configured there in the active experiment tree.
...
- `agcrn_official_aligned` still consumes the original 9 experiment protocols only. The masked protocol is wired only into `agcrn_masked`, and `world_model_v1` is wired into both `world_model_agcrn_v1` and `world_model_rollout_v1`.
