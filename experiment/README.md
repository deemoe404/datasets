...
- [world_model_baselines_v1](./families/world_model_baselines_v1/README.md): `kelmarsh` only, farm-synchronous, persistence plus shared-weight TFT-no-graph, TimeXer-no-graph, DGCRN-style dynamic-graph, Chronos-2 zero-shot, iTransformer-no-graph, and MTGNN calendar-graph baselines (`world_model_v1`)
