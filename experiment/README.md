...
- `agcrn_official_aligned` still consumes the original 9 experiment protocols only. The dataset-side `power_wd_yaw_pmean_hist_sincos_masked` protocol is wired only into the dedicated `agcrn_masked` family, not retrofitted into the baseline AGCRN runner. `world_model_v1` currently exists only at the dataset/task-bundle layer and is not wired into any active experiment family.
