...
MODEL_VARIANT = "masked_power_wd_yaw_pmean_hist_sincos_farm_sync"
...
PROFILE_LOG_PREFIX = "[agcrn_masked] "
...
_RUN_AGCRN_WORK_ROOT = EXPERIMENT_DIR / ".work" / "run_agcrn_masked"
_RUN_STATE_SCHEMA_VERSION = "agcrn_masked.run_agcrn.resume.v1"
_TRAINING_CHECKPOINT_SCHEMA_VERSION = "agcrn_masked.training_checkpoint.v1"
...
def require_torch() -> tuple[Any, Any, Any, Any, Any]:
    if torch is None or nn is None or F is None or DataLoader is None or Dataset is None:
        raise ImportError(
            ...
            "Create experiment/families/agcrn_masked/.conda with ./create_env.sh."
        )
...
def build_source_tensor(
    ...
    *,
    ...
    if len(train_windows) == 0:
        raise ValueError("At least one train window is required to prepare AGCRN masked inputs.")
...
def _finalize_prepared_dataset(
    ...
    *,
    ...
    train_windows = filter_windows_with_valid_targets(
    ...
    train_windows = limit_windows(train_windows, max_windows=max_train_origins)
...
if Dataset is not None:
    ...
    class PanelWindowDataset(Dataset):
        ...
        def __getitem__(self, index: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
            ...
            if not np.isfinite(history).all() or not np.isfinite(targets).all() or not np.isfinite(target_valid_mask).all():
    ...
    *,
    ...
    try:
        with resolved_torch.no_grad():
            for batch_history, batch_targets, batch_target_valid_mask in loader:
                ...
                batch_target_valid_mask = batch_target_valid_mask.to(device=device, dtype=resolved_torch.float32)
        ...
        for epoch_index in range(start_epoch, max_epochs + 1):
            ...
            try:
                for batch_history, batch_targets, batch_target_valid_mask in train_loader:
                    ...
                    batch_target_valid_mask = batch_target_valid_mask.to(device=resolved_device, dtype=resolved_torch.float32)
...
def run_experiment(
    *,
    ...
    job_progress = _create_progress_bar(total=total_jobs, desc="agcrn_masked jobs", leave=True)
    try:
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the masked AGCRN smoke-test family on kelmarsh and penmanshiel."
    )
    parser.add_argument(
        "--dataset",
        action="append",
    ...
    parser.add_argument(
        ...
        dest="variants",
        help="Limit execution to the masked model variant. Defaults to running the only active variant.",
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional label suffix for the formal run record under experiment/artifacts/runs/agcrn_masked/.",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume an interrupted run_agcrn_masked invocation from experiment/families/agcrn_masked/.work/.",
    )
    parser.add_argument(
        "--force-rerun",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.no_record_run:
        ...
        record_cli_run(
            ...
            entrypoint="experiment/families/agcrn_masked/run_agcrn_masked.py",
            args=recorded_args,
            output_path=resolved_output_path,
