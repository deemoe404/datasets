...
import world_model_agcrn_v1 as agcrn
...
ALIGNMENT_VARIANTS = agcrn.resolve_variant_specs((agcrn.MODEL_VARIANT,))
...
COMMON_SCREEN_CONFIGS = (
    SearchConfig(
        name="baseline_bs512_h64_e16_l2_k2_lr1e-3_tf0p50_wd0",
        batch_size=512,
        learning_rate=1e-3,
        hidden_dim=64,
        embed_dim=16,
    ...
    SearchConfig(
        name="baseline_bs512_h64_e16_l2_k2_lr5e-4_tf0p50_wd0",
        batch_size=512,
        learning_rate=5e-4,
        hidden_dim=64,
        embed_dim=16,
    ...
    SearchConfig(
        name="baseline_bs512_h64_e16_l2_k2_lr1e-3_tf0p25_wd1e-5",
        batch_size=512,
        learning_rate=1e-3,
        hidden_dim=64,
        embed_dim=16,
    ...
    SearchConfig(
        name="graph_bs512_h64_e24_l2_k3_lr5e-4_tf0p50_wd0",
        batch_size=512,
        learning_rate=5e-4,
        hidden_dim=64,
        embed_dim=24,
    ...
    SearchConfig(
        name="graph_bs512_h64_e24_l2_k3_lr5e-4_tf0p25_wd1e-5",
        batch_size=512,
        learning_rate=5e-4,
        hidden_dim=64,
        embed_dim=24,
    ...
    SearchConfig(
        name="graph_bs512_h64_e24_l2_k3_lr3e-4_tf0p25_wd1e-4",
        batch_size=512,
        learning_rate=3e-4,
        hidden_dim=64,
        embed_dim=24,
    ...
    SearchConfig(
        name="larger_bs512_h96_e24_l2_k3_lr5e-4_tf0p25_wd1e-5",
        batch_size=512,
        learning_rate=5e-4,
        hidden_dim=96,
        embed_dim=24,
    ...
    SearchConfig(
        name="compact_bs512_h48_e12_l1_k2_lr1e-3_tf0p25_wd1e-5",
        batch_size=512,
        learning_rate=1e-3,
        hidden_dim=48,
        embed_dim=12,
...
SCREEN_CONFIGS_BY_VARIANT = {
    agcrn.MODEL_VARIANT: COMMON_SCREEN_CONFIGS,
}
...
def _is_oom_error(exc: BaseException) -> bool:
    if agcrn.torch is not None:
        oom_error = getattr(agcrn.torch, "OutOfMemoryError", None)
...
def _config_from_row(row: dict[str, object]) -> SearchConfig:
    return SearchConfig(
        ...
        teacher_forcing_ratio=float(row.get("teacher_forcing_ratio", agcrn.DEFAULT_TEACHER_FORCING_RATIO)),
        weight_decay=float(row.get("weight_decay", agcrn.DEFAULT_WEIGHT_DECAY)),
    )


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the world-model AGCRN search harness on kelmarsh and penmanshiel.",
    )
    parser.add_argument(
        "--dataset",
        action="append",
    ...
    parser.add_argument(
        ...
        dest="variants",
        help="Limit the tuned variants. Defaults to the only active world-model variant.",
    )
    parser.add_argument(
        "--config-name",
        action="append",
    ...
    parser.add_argument(
        ...
        type=int,
        default=16_384,
        help="Dense train-origin cap used during screening.",
    )
    parser.add_argument(
        "--screen-eval-origins",
        type=int,
        default=2_048,
        help="Dense val-origin cap used during screening.",
    )
    parser.add_argument(
        "--screen-epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="How many screened configs per dataset to carry into the final stage.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--skip-final",
        action="store_true",
        help="Only run screening and skip the full-window confirmation stage.",
    )
    parser.add_argument(
        "--full-only",
        action="store_true",
    ...
    *,
    ...
    _write_json(
        output_dir / "search_plan.json",
        {
            ...
            "device": device,
            "screen_configs": {
                variant_name: [asdict(config) for config in selected_configs_by_variant[variant_name]]
                for variant_name in tuned_variants
            },
            "full_only": full_only,
        },
        ...
        for variant_name in tuned_variants:
            ...
            for config in configs_for_final:
                try:
                    ...
                    _emit_oom_event(
                        stage="final",
                        prepared_dataset=prepared_full[variant_name],
                        config=config,
                        exc=exc,
                    )
                    continue
    ...
    final_frame = _sort_final_rows(pl.DataFrame(final_rows))
