...
import experiment.infra.common.window_protocols as window_protocols  # noqa: E402
...
MODEL_VARIANT = "world_model_v1_seq2seq_farm_sync"
...
GRAPH_MODE = "adaptive"
GRAPH_SOURCE = "pairwise_biased_adaptive_graph"
INPUT_CHANNELS = 52
OUTPUT_CHANNELS = 1
...
DEFAULT_MAX_CUDA_EVAL_BATCH_SIZE = 2048
DEFAULT_CUDA_NUM_WORKERS = 4
DEFAULT_CUDA_PREFETCH_FACTOR = 4
PROFILE_LOG_PREFIX = "[world_model_agcrn_v1] "
...
PAST_COVARIATE_VALUE_COUNT = 25
PAST_COVARIATE_COUNT = PAST_COVARIATE_VALUE_COUNT * 2
...
PAIRWISE_SOURCE_NUMERIC_COLUMNS = (
    "delta_x_m",
    "delta_y_m",
    "distance_m",
    "bearing_deg",
...
_RUN_AGCRN_WORK_ROOT = EXPERIMENT_DIR / ".work" / "run_world_model_agcrn_v1"
_RUN_STATE_SCHEMA_VERSION = "world_model_agcrn_v1.run.resume.v1"
_TRAINING_CHECKPOINT_SCHEMA_VERSION = "world_model_agcrn_v1.training_checkpoint.v1"
...
_LARGER_BS256_LR5E4_HYPERPARAMETERS = HyperparameterProfile(
    batch_size=256,
    learning_rate=5e-4,
    max_epochs=20,
    early_stopping_patience=5,
...
TUNED_DEFAULT_HYPERPARAMETERS_BY_DATASET_AND_VARIANT = {
    ...
    "penmanshiel": {
        MODEL_VARIANT: _GRAPH_BS512_LR5E4_HYPERPARAMETERS,
    },
}
...
def resolve_hyperparameter_profile(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            f"Unsupported world-model AGCRN dataset/variant pair {dataset_id!r}/{variant_name!r}."
        ) from exc
    return HyperparameterProfile(
        ...
        teacher_forcing_ratio=(
            defaults.teacher_forcing_ratio
...
class PreparedDataset:
    ...
    known_future_channel_names: tuple[str, ...]
    static_feature_names: tuple[str, ...]
    pairwise_feature_names: tuple[str, ...]
    source_tensor: np.ndarray
    known_future_tensor: np.ndarray
    ...
    @property
    def known_future_channels(self) -> int:
        return int(self.known_future_tensor.shape[1])
...
def require_torch() -> tuple[Any, Any, Any, Any, Any]:
    if torch is None or nn is None or F is None or DataLoader is None or Dataset is None:
        raise ImportError(
            ...
            "Create experiment/families/world_model_agcrn_v1/.conda with ./create_env.sh."
        )
...
def _configure_torch_runtime(*, device: str, torch_module) -> None:
    if resolve_device(device, torch_module=torch_module) != "cuda":
        return
...
def resolve_past_covariate_columns(bundle: Any) -> tuple[str, ...]:
    ...
    raw_columns = tuple(str(column) for column in (column_groups.get("past_covariates") or ()))
    if len(raw_columns) != PAST_COVARIATE_COUNT:
        raise ValueError(
            "Task bundle task_context past_covariates do not match the expected "
            "world_model_v1 contract: expected 25 value columns followed by 25 companion masks."
        )
    value_columns = raw_columns[:PAST_COVARIATE_VALUE_COUNT]
    mask_columns = raw_columns[PAST_COVARIATE_VALUE_COUNT:]
    if any(column.endswith("__mask") for column in value_columns):
        raise ValueError("Task bundle task_context past_covariates values must appear before companion masks.")
    expected_mask_columns = tuple(f"{column}__mask" for column in value_columns)
    if mask_columns != expected_mask_columns:
        raise ValueError(
            "Task bundle task_context past_covariates do not preserve the expected "
            "value-column then companion-mask ordering for world_model_v1."
        )
    return raw_columns
...
def resolve_target_history_mask_columns(bundle: Any) -> tuple[str, ...]:
    ...
    if not isinstance(column_groups, dict):
        raise ValueError("Task bundle task_context is missing column_groups.")
    raw_columns = tuple(str(column) for column in (column_groups.get("target_history_masks") or ()))
    if raw_columns != EXPECTED_TARGET_HISTORY_MASK_COLUMNS:
        raise ValueError(
            "Task bundle task_context target_history_masks do not match the expected world_model_v1 contract."
        )
    return raw_columns


def resolve_known_future_feature_columns(bundle: Any) -> tuple[str, ...]:
    column_groups = bundle.task_context.get("column_groups", {})
    if not isinstance(column_groups, dict):
        raise ValueError("Task bundle task_context is missing column_groups.")
    raw_columns = tuple(str(column) for column in (column_groups.get("known_future") or ()))
    expected_columns = ("dataset", "timestamp", *KNOWN_FUTURE_FEATURE_COLUMNS)
    if raw_columns != expected_columns:
        raise ValueError(
            "Task bundle task_context known_future columns do not match the expected world_model_v1 contract."
        )
    return KNOWN_FUTURE_FEATURE_COLUMNS


def resolve_static_feature_names(turbine_static: pl.DataFrame) -> tuple[str, ...]:
    coordinate_columns = resolve_static_coordinate_columns(turbine_static)
    return (*coordinate_columns, "elevation_m", "rated_power_kw", "hub_height_m", "rotor_diameter_m")


def resolve_pairwise_feature_names(bundle: Any) -> tuple[str, ...]:
    column_groups = bundle.task_context.get("column_groups", {})
    if not isinstance(column_groups, dict):
        raise ValueError("Task bundle task_context is missing column_groups.")
    raw_columns = tuple(str(column) for column in (column_groups.get("pairwise") or ()))
    expected_prefix = (
        "src_turbine_id",
        "dst_turbine_id",
        "src_turbine_index",
    ...
    if raw_columns != expected_prefix:
        raise ValueError(
            "Task bundle task_context pairwise columns do not match the expected world_model_v1 contract."
        )
    return PAIRWISE_NUMERIC_COLUMNS


def load_known_future_frame(
    dataset_id: str,
    bundle: Any,
    *,
    known_future_feature_columns: Sequence[str],
) -> pl.DataFrame:
    known_future = getattr(bundle, "known_future", None)
    if known_future is None:
        raise ValueError(f"Task bundle for dataset {dataset_id!r} is missing known_future sidecar.")
    required_columns = ("dataset", "timestamp", *known_future_feature_columns)
    available_columns = set(known_future.columns)
    missing_columns = [column for column in required_columns if column not in available_columns]
    if missing_columns:
        raise ValueError(
            f"Task bundle known_future for dataset {dataset_id!r} is missing required columns {missing_columns!r}."
        )
    frame = known_future.select(list(required_columns)).sort("timestamp")
    if frame.is_empty():
        raise ValueError(f"Task bundle known_future for dataset {dataset_id!r} is empty.")
    return frame


def load_pairwise_frame(dataset_id: str, bundle: Any) -> pl.DataFrame:
    pairwise = getattr(bundle, "pairwise", None)
    if pairwise is None:
        raise ValueError(f"Task bundle for dataset {dataset_id!r} is missing pairwise sidecar.")
    required_columns = (
        "src_turbine_id",
        "dst_turbine_id",
        "src_turbine_index",
    ...
    available_columns = set(pairwise.columns)
    missing_columns = [column for column in required_columns if column not in available_columns]
    if missing_columns:
        raise ValueError(
            f"Task bundle pairwise for dataset {dataset_id!r} is missing required columns {missing_columns!r}."
        )
    frame = pairwise.select(list(required_columns)).sort(["src_turbine_index", "dst_turbine_index"])
    if frame.is_empty():
        raise ValueError(f"Task bundle pairwise for dataset {dataset_id!r} is empty.")
    return frame
...
def load_window_index(dataset_id: str, bundle: Any) -> pl.DataFrame:
    ...
    load_started = time.monotonic()
    frame = bundle.window_index.select(list(_TASK_WINDOW_COLUMNS)).sort("output_start_ts")
    _profile_log(
        dataset_id,
        "load_window_index",
        window_count=frame.height,
        duration_seconds=round(time.monotonic() - load_started, 6),
    )
    if frame.is_empty():
        raise ValueError(f"Dataset {dataset_id!r} has no task windows for {TASK_ID}.")
    return frame
...
def build_target_valid_mask(target_pu: np.ndarray) -> np.ndarray:
    return np.isfinite(np.asarray(target_pu, dtype=np.float32))


def build_known_future_tensor(
    known_future: pl.DataFrame,
    *,
    expected_timestamps: Sequence[datetime],
    known_future_feature_columns: Sequence[str],
) -> np.ndarray:
    if known_future["timestamp"].to_list() != list(expected_timestamps):
        raise ValueError("Known-future timestamp axis does not match the task bundle panel axis.")
    matrix = known_future.select(list(known_future_feature_columns)).to_numpy().astype(np.float32, copy=False)
    if not np.isfinite(matrix).all():
        raise ValueError("Known-future tensor contains null or non-finite values.")
    return matrix


def _normalize_dense_feature_matrix(
    matrix: np.ndarray,
    *,
    ...
    if normalized.ndim != 2:
        raise ValueError(f"{context_name} expected a dense 2D feature matrix, got shape {normalized.shape!r}.")
    for column_index, column_name in enumerate(column_names):
        values = normalized[:, column_index]
        finite_values = values[np.isfinite(values)]
        if finite_values.size == 0:
            raise ValueError(f"{context_name} column {column_name!r} has no finite values.")
        mean = float(finite_values.mean())
        std = float(finite_values.std())
        if np.isfinite(std) and std > 0:
            normalized[:, column_index] = (values - mean) / std
        else:
            normalized[:, column_index] = 0.0
    if not np.isfinite(normalized).all():
        raise ValueError(f"{context_name} normalization produced non-finite values.")
...
def build_static_tensor(
    ...
    *,
    ...
    ordered = turbine_static.sort("turbine_index")
    available_columns = set(ordered.columns)
    missing_columns = [column for column in feature_names if column not in available_columns]
    if missing_columns:
        raise ValueError(f"Task bundle static sidecar is missing required columns {missing_columns!r}.")
    frame = ordered.select(list(feature_names))
    if any(frame[column].null_count() > 0 for column in feature_names):
        incomplete = [column for column in feature_names if frame[column].null_count() > 0]
        raise ValueError(f"Task bundle static sidecar has null values in required columns {incomplete!r}.")
    return _normalize_dense_feature_matrix(
        frame.to_numpy().astype(np.float32, copy=False),
        column_names=feature_names,
        context_name="static tensor",
    )


def build_pairwise_tensor(
    pairwise: pl.DataFrame,
    *,
    turbine_ids: Sequence[str],
    pairwise_feature_names: Sequence[str],
) -> np.ndarray:
    node_count = len(turbine_ids)
    turbine_index_by_id = {
        turbine_id: turbine_index
        for turbine_index, turbine_id in enumerate(turbine_ids)
    }
    pairwise_frame = pairwise.with_columns(
    ...
    feature_frame = pairwise_frame.select(list(PAIRWISE_NUMERIC_COLUMNS))
    if any(feature_frame[column].null_count() > 0 for column in PAIRWISE_NUMERIC_COLUMNS):
        incomplete = [column for column in PAIRWISE_NUMERIC_COLUMNS if feature_frame[column].null_count() > 0]
        raise ValueError(f"Task bundle pairwise sidecar has null values in required columns {incomplete!r}.")
    normalized_features = _normalize_dense_feature_matrix(
        feature_frame.to_numpy().astype(np.float32, copy=False),
        column_names=pairwise_feature_names,
        context_name="pairwise tensor",
    )
    ...
    for row_index, row in enumerate(pairwise_frame.iter_rows(named=True)):
        ...
        if not (0 <= src_index < node_count and 0 <= dst_index < node_count):
            raise ValueError("Pairwise sidecar contains out-of-range turbine indices.")
        if (src_index, dst_index) in coverage:
            raise ValueError("Pairwise sidecar contains duplicate directed turbine pairs.")
        tensor[src_index, dst_index, :] = normalized_features[row_index]
        coverage.add((src_index, dst_index))
    expected_coverage = {
        (src_index, dst_index)
...
def _load_variant_dataset_context(
    ...
    *,
    ...
    known_future_feature_columns = resolve_known_future_feature_columns(bundle)
    pairwise_feature_names = resolve_pairwise_feature_names(bundle)
    static_feature_names = resolve_static_feature_names(metadata.turbine_static)
    window_index = load_window_index(dataset_id, bundle)
    ...
    known_future = load_known_future_frame(
        dataset_id,
        bundle,
        known_future_feature_columns=known_future_feature_columns,
    )
    pairwise = load_pairwise_frame(dataset_id, bundle)
...
def _is_mask_channel_name(column_name: str) -> bool:
    return column_name.endswith("__mask")


def _compute_train_channel_stats(
    channel_values: np.ndarray,
    *,
    history_mask: np.ndarray,
    column_name: str,
) -> tuple[np.float32, np.float32]:
    train_history = channel_values[history_mask]
    finite_values = train_history[np.isfinite(train_history)]
    if finite_values.size == 0:
        raise ValueError(f"Channel {column_name!r} has no finite training values for normalization.")
    mean = np.float32(float(finite_values.mean()))
    std = float(finite_values.std())
    resolved_std = np.float32(std if np.isfinite(std) and std > 0 else 1.0)
    return mean, resolved_std


def _normalize_value_channel(
    channel_values: np.ndarray,
    *,
    history_mask: np.ndarray,
    column_name: str,
) -> np.ndarray:
    mean, std = _compute_train_channel_stats(
        channel_values,
        history_mask=history_mask,
        column_name=column_name,
    )
    normalized = ((channel_values - mean) / std).astype(np.float32, copy=False)
    normalized[~np.isfinite(normalized)] = 0.0
    return normalized


def _prepare_mask_channel(channel_values: np.ndarray, *, column_name: str) -> np.ndarray:
    channel = np.asarray(channel_values, dtype=np.float32)
    if not np.isfinite(channel).all():
        raise ValueError(f"Mask channel {column_name!r} must not contain null or non-finite values.")
    unique_values = np.unique(channel)
    if not set(unique_values.tolist()).issubset({0.0, 1.0}):
        raise ValueError(f"Mask channel {column_name!r} must remain raw 0/1 values.")
...
def build_source_tensor(
    ...
    *,
    ...
    if len(train_windows) == 0:
        raise ValueError("At least one train window is required to prepare world-model AGCRN inputs.")
    history_mask = _history_row_mask(
        target_indices=train_windows.target_indices,
        history_steps=history_steps,
        total_steps=target_pu.shape[0],
    )
    source_channels = [np.where(np.isfinite(target_pu), target_pu, 0.0).astype(np.float32, copy=False)]
    input_channel_names: list[str] = ["target_pu"]
    for feature_column in (*target_history_mask_columns, *past_covariate_columns):
        ...
        prepared_channel = (
            _prepare_mask_channel(panel, column_name=feature_column)
            if _is_mask_channel_name(feature_column)
            else _normalize_value_channel(
                panel,
                history_mask=history_mask,
                column_name=feature_column,
            )
        )
        source_channels.append(prepared_channel)
        input_channel_names.append(feature_column)
...
def _take_window_indices(windows: FarmWindowDescriptorIndex, keep_indices: np.ndarray) -> FarmWindowDescriptorIndex:
...
def filter_windows_with_valid_targets(
    ...
    *,
    ...
    keep_indices = np.asarray(
        [
            index
            for index, target_index in enumerate(windows.target_indices.tolist())
            if int(target_valid_mask[target_index : target_index + forecast_steps].sum()) > 0
        ],
        dtype=np.int64,
    )
    return _take_window_indices(windows, keep_indices)


def limit_windows(windows: FarmWindowDescriptorIndex, *, max_windows: int | None) -> FarmWindowDescriptorIndex:
    if max_windows is None or len(windows) <= max_windows:
        return windows
    keep_indices = np.arange(0, max_windows, dtype=np.int64)
    return _take_window_indices(windows, keep_indices)
...
def _align_split_frames(
    ...
    for split_name in _SPLIT_NAMES:
        ...
        if shared_keys is None or shared_keys.is_empty():
            raise ValueError(
                f"Dataset {contexts[0].dataset_id!r} has no shared windows for split {split_name!r} "
...
def _finalize_prepared_dataset(
    ...
    *,
    ...
    target_valid_mask = build_target_valid_mask(context.target_pu)
    full_train_windows = filter_windows_with_valid_targets(
        build_window_descriptor_index(split_frames["train"], timestamps_us=context.timestamps_us),
        target_valid_mask=target_valid_mask,
        forecast_steps=FORECAST_STEPS,
    )
    val_rolling_windows = filter_windows_with_valid_targets(
        build_window_descriptor_index(split_frames["val"], timestamps_us=context.timestamps_us),
        target_valid_mask=target_valid_mask,
        forecast_steps=FORECAST_STEPS,
    )
    test_rolling_windows = filter_windows_with_valid_targets(
        build_window_descriptor_index(split_frames["test"], timestamps_us=context.timestamps_us),
        target_valid_mask=target_valid_mask,
        forecast_steps=FORECAST_STEPS,
    )
    train_windows = limit_windows(full_train_windows, max_windows=max_train_origins)
    val_rolling_windows = limit_windows(val_rolling_windows, max_windows=max_eval_origins)
    test_rolling_windows = limit_windows(test_rolling_windows, max_windows=max_eval_origins)
    if len(full_train_windows) == 0:
        raise ValueError(f"Dataset {context.dataset_id!r} has no train windows with at least one valid target.")
    if len(val_rolling_windows) == 0:
        raise ValueError(f"Dataset {context.dataset_id!r} has no val windows with at least one valid target.")
    if len(test_rolling_windows) == 0:
        raise ValueError(f"Dataset {context.dataset_id!r} has no test windows with at least one valid target.")
    ...
    source_tensor, input_channel_names = build_source_tensor(
        ...
        train_windows=full_train_windows,
        history_steps=HISTORY_STEPS,
    )
    known_future_tensor = build_known_future_tensor(
    ...
    static_tensor = build_static_tensor(
        context.metadata.turbine_static,
        feature_names=context.static_feature_names,
    )
    pairwise_tensor = build_pairwise_tensor(
        context.pairwise,
        turbine_ids=context.metadata.turbine_ids,
        pairwise_feature_names=context.pairwise_feature_names,
    )
    ...
    _profile_log(
        ...
        known_future_channels=known_future_tensor.shape[1],
        model_variant=context.model_variant,
        node_count=len(context.metadata.turbine_ids),
        pairwise_features=pairwise_tensor.shape[2],
        static_features=static_tensor.shape[1],
    ...
    return PreparedDataset(
        ...
        known_future_channel_names=context.known_future_feature_columns,
        static_feature_names=context.static_feature_names,
        pairwise_feature_names=context.pairwise_feature_names,
    ...
    class PanelWindowDataset(Dataset):
        def __init__(
            ...
            *,
            ...
            self.known_future_tensor = np.asarray(known_future_tensor, dtype=np.float32)
        ...
        def __getitem__(self, index: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
            ...
            known_future = self.known_future_tensor[target_index : target_index + self.forecast_steps]
            ...
            if known_future.shape != (self.forecast_steps, self.known_future_tensor.shape[1]):
                raise ValueError(f"Known-future slice for index {index} has unexpected shape {known_future.shape!r}.")
            ...
            if target_valid_mask.shape != (self.forecast_steps, self.target_valid_mask.shape[1]):
                raise ValueError(f"Target valid mask slice for index {index} has unexpected shape {target_valid_mask.shape!r}.")
            if (
                not np.isfinite(history).all()
                or not np.isfinite(known_future).all()
                or not np.isfinite(targets).all()
                or not np.isfinite(target_valid_mask).all()
            ):
                raise ValueError(f"Window index {index} contains non-finite values despite masking and zero-fill.")
            return (
                history.astype(np.float32, copy=True),
                known_future.astype(np.float32, copy=True),
...
if nn is not None and F is not None:
    ...
    class AVWGCN(nn.Module):
        ...
        def forward(self, x, node_embeddings, geometry_bias):
            node_count = node_embeddings.shape[0]
            supports = F.softmax(
                F.relu(torch.mm(node_embeddings, node_embeddings.transpose(0, 1)) + geometry_bias),
                dim=1,
            )
    ...
    class AGCRNCell(nn.Module):
        ...
        def forward(self, x, state, node_embeddings, geometry_bias):
            state = state.to(device=x.device, dtype=x.dtype)
            input_and_state = torch.cat((x, state), dim=-1)
            z_r = torch.sigmoid(self.gate(input_and_state, node_embeddings, geometry_bias))
            z, r = torch.split(z_r, self.hidden_dim, dim=-1)
            candidate = torch.cat((x, z * state), dim=-1)
            hc = torch.tanh(self.update(candidate, node_embeddings, geometry_bias))
    ...
    class AVWDCRNN(nn.Module):
        ...
        def forward(self, x, init_state, node_embeddings, geometry_bias):
    ...
    class AGCRN(nn.Module):
        def __init__(
            ...
            *,
            ...
            self.known_future_channels = known_future_channels
            self.hidden_dim = hidden_dim
            ...
            self.residual_node_embeddings = nn.Parameter(torch.randn(self.num_node, embed_dim), requires_grad=True)
            self.static_encoder = nn.Sequential(
                nn.Linear(static_features.shape[1], STATIC_HIDDEN_DIM),
                nn.ReLU(),
                nn.Linear(STATIC_HIDDEN_DIM, embed_dim),
            )
            self.pairwise_encoder = nn.Sequential(
                nn.Linear(pairwise_features.shape[2], PAIRWISE_HIDDEN_DIM),
                nn.ReLU(),
            ...
            self.decoder = nn.ModuleList(
                [
                    AGCRNCell(
                        self.num_node,
                        1 + self.known_future_channels if layer_index == 0 else self.hidden_dim,
                        self.hidden_dim,
                        cheb_k,
                        embed_dim,
                    )
        ...
        def compute_graph_context(self):
            node_embeddings = self.static_encoder(self.static_features) + self.residual_node_embeddings
            geometry_bias = self.pairwise_encoder(self.pairwise_features).squeeze(-1)
            supports = F.softmax(
                F.relu(torch.mm(node_embeddings, node_embeddings.transpose(0, 1)) + geometry_bias),
                dim=1,
            )
            return node_embeddings, geometry_bias, supports
        ...
        def forward(
            ...
            if known_future.shape[1] != self.horizon:
                raise ValueError(f"Expected forecast_steps={self.horizon}, received {known_future.shape[1]}.")
            if known_future.shape[2] != self.known_future_channels:
                raise ValueError(
                    f"Expected known_future_channels={self.known_future_channels}, received {known_future.shape[2]}."
                )

            node_embeddings, geometry_bias, _supports = self.compute_graph_context()
            init_state = self.encoder.init_hidden(source.shape[0]).to(device=source.device, dtype=source.dtype)
            _encoder_output, encoder_state = self.encoder(source, init_state, node_embeddings, geometry_bias)
            decoder_states = [encoder_state[layer_index] for layer_index in range(self.num_layers)]
            previous_target = source[:, -1, :, :1]
            outputs: list[Any] = []
            for horizon_index in range(self.horizon):
                step_known_future = known_future[:, horizon_index, :]
                step_known_future = step_known_future[:, None, :].expand(-1, self.num_node, -1)
                current_inputs = torch.cat((previous_target, step_known_future), dim=-1)
                next_states: list[Any] = []
                for layer_index, cell in enumerate(self.decoder):
                    state = cell(current_inputs, decoder_states[layer_index], node_embeddings, geometry_bias)
...
def build_model(
    *,
    ...
    return AGCRN(
        ...
        known_future_channels=known_future_channels,
        static_tensor=static_tensor,
        pairwise_tensor=pairwise_tensor,
        hidden_dim=hidden_dim,
        forecast_steps=forecast_steps,
...
def initialize_agcrn_parameters(model) -> None:
...
def _build_dataloader(
    ...
    *,
    ...
    resolved_device = resolve_device(device, torch_module=resolved_torch)
    ...
    dataset = PanelWindowDataset(
        ...
        prepared_dataset.known_future_tensor,
...
def masked_mse_loss(predictions, targets, valid_mask, *, torch_module) -> Any:
    ...
    if float(valid_count.item()) <= 0:
        raise ValueError("masked_mse_loss received a batch with zero valid targets.")
    squared_errors = torch_module.square(predictions - targets) * resolved_mask
    return squared_errors.sum() / valid_count
    ...
    *,
    ...
    try:
        with resolved_torch.no_grad():
            for batch_history, batch_known_future, batch_targets, batch_target_valid_mask in loader:
                batch_history = batch_history.to(
                    device=device,
                    dtype=resolved_torch.float32,
                    non_blocking=device == "cuda",
                )
                batch_known_future = batch_known_future.to(
                    device=device,
                    dtype=resolved_torch.float32,
                    non_blocking=device == "cuda",
                )
...
def train_model(
    ...
    *,
    ...
    resolved_device = resolve_device(device)
    _configure_torch_runtime(device=resolved_device, torch_module=resolved_torch)
    ...
    model = build_model(
        ...
        known_future_channels=prepared_dataset.known_future_channels,
    ...
    initialize_agcrn_parameters(model)
    optimizer = resolved_torch.optim.Adam(
        model.parameters(),
        lr=learning_rate,
        weight_decay=weight_decay,
    )
    ...
    try:
        ...
        for epoch_index in range(start_epoch, max_epochs + 1):
            ...
            train_batch_count = 0
            try:
                for batch_history, batch_known_future, batch_targets, batch_target_valid_mask in train_loader:
                    batch_history = batch_history.to(
                        device=resolved_device,
                        dtype=resolved_torch.float32,
...
def run_experiment(
    *,
    ...
    job_progress = _create_progress_bar(total=total_jobs, desc="world_model_agcrn_v1 jobs", leave=True)
    try:
        for dataset_id in dataset_ids:
            ...
            for prepared in prepared_datasets:
                ...
                rows.extend(
                    runner(
                        ...
                        teacher_forcing_ratio=resolved_profile.teacher_forcing_ratio,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the world-model AGCRN family on kelmarsh and penmanshiel."
    )
    parser.add_argument(
        "--dataset",
        action="append",
    ...
    parser.add_argument(
        ...
        dest="variants",
        help="Limit execution to the active world-model AGCRN variant. Defaults to running the only active variant.",
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Training batch size. Defaults to the tuned per-variant profile when omitted.",
    )
    parser.add_argument(
        "--eval-batch-size",
        type=int,
        default=None,
        help="Evaluation batch size. Defaults to an auto-scaled larger CUDA batch when omitted.",
    )
    parser.add_argument(
        "--learning-rate",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Teacher forcing ratio used during seq2seq decoding. Defaults to the tuned per-variant profile when omitted.",
    )
    parser.add_argument(
        "--weight-decay",
        type=float,
        default=None,
        help="Adam weight decay. Defaults to the tuned per-variant profile when omitted.",
    )
    parser.add_argument(
        "--num-workers",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional label suffix for the formal run record under experiment/artifacts/runs/world_model_agcrn_v1/.",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume an interrupted run_world_model_agcrn_v1 invocation from experiment/families/world_model_agcrn_v1/.work/.",
    )
    parser.add_argument(
        "--force-rerun",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.no_record_run:
        ...
        record_cli_run(
            ...
            entrypoint="experiment/families/world_model_agcrn_v1/run_world_model_agcrn_v1.py",
            args=recorded_args,
            output_path=resolved_output_path,
