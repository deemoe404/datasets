...
ITRANSFORMER_ENCODER_LAYERS = 3
ITRANSFORMER_FFN_MULTIPLIER = 4
...
_RUN_STATE_SCHEMA_VERSION = "world_model_baselines_v1.run.resume.v2"
_TRAINING_CHECKPOINT_SCHEMA_VERSION = "world_model_baselines_v1.training_checkpoint.v2"
...
SELECTION_METRIC_RMSE = "val_rmse_pu"
SELECTION_METRIC_MAE = "val_mae_pu"
DEFAULT_SELECTION_METRICS = (
    SELECTION_METRIC_RMSE,
    SELECTION_METRIC_MAE,
)
_SELECTION_METRIC_ORDER = {
    SELECTION_METRIC_RMSE: 0,
    SELECTION_METRIC_MAE: 1,
}
...
def resolve_selection_metrics(selection_metrics: Sequence[str] | None) -> tuple[str, ...]:
    if not selection_metrics:
        return DEFAULT_SELECTION_METRICS
    resolved: list[str] = []
    for selection_metric in selection_metrics:
        if selection_metric not in _SELECTION_METRIC_ORDER:
            raise ValueError(
                f"Unsupported selection_metric {selection_metric!r}. "
                f"Expected one of {sorted(_SELECTION_METRIC_ORDER)!r}."
            )
        if selection_metric not in resolved:
            resolved.append(selection_metric)
...
def _selection_metric_value(
    ...
    *,
    ...
    if selection_metric == SELECTION_METRIC_MAE:
        return float(val_mae_pu)
    if selection_metric == SELECTION_METRIC_RMSE:
        return float(val_rmse_pu)
    raise ValueError(f"Unsupported selection_metric {selection_metric!r}.")


def _selection_metric_label(selection_metric: str) -> str:
    return selection_metric.replace("_pu", "")


def _selection_metric_improved(
    selection_metric: str,
    *,
    val_mae_pu: float,
    val_rmse_pu: float,
    ...
    current_value = _selection_metric_value(
        selection_metric,
        val_mae_pu=val_mae_pu,
        val_rmse_pu=val_rmse_pu,
    )
    best_value = _selection_metric_value(
        selection_metric,
        val_mae_pu=best_val_mae_pu,
        val_rmse_pu=best_val_rmse_pu,
    )
    return math.isfinite(current_value) and (not math.isfinite(best_value) or current_value < best_value - 1e-12)
...
VARIANT_SPECS = (
    ...
    ExperimentVariant(
        model_variant=ITRANSFORMER_VARIANT,
        feature_protocol_id=FEATURE_PROTOCOL_ID,
...
_DEFAULT_ITRANSFORMER_PROFILE = HyperparameterProfile(
    batch_size=64,
    learning_rate=1e-4,
    max_epochs=30,
...
def resolve_hyperparameter_profile(
    ...
    *,
    ...
    if variant_name in {TFT_VARIANT, TIMEXER_VARIANT, ITRANSFORMER_VARIANT}:
...
def _tensorboard_job_log_dir(
    ...
    *,
    ...
    return Path(tensorboard_root) / dataset_id / model_variant / selection_metric
...
if Dataset is not None:
    ...
    class ITransformerWindowDataset(Dataset):
        def __init__(
            self,
            prepared_dataset: state_base.PreparedDataset,
            windows: world_model_base.FarmWindowDescriptorIndex,
        ...
        def __getitem__(self, index: int):
            ...
            item = (
                ...
                prepared.context_history_tensor[history_slice, :].astype(np.float32, copy=True),
                prepared.context_future_tensor[future_slice, :].astype(np.float32, copy=True),
                prepared.static_tensor[:, :].astype(np.float32, copy=True),
...
def _build_itransformer_dataloader(
    prepared_dataset: state_base.PreparedDataset,
    *,
    windows: world_model_base.FarmWindowDescriptorIndex,
    batch_size: int,
...
if nn is not None and F is not None:
    ...
    class ITransformerNoGraph(nn.Module):
        def __init__(
            self,
            *,
            history_steps: int,
            ...
            dropout: float,
            bounded_output_epsilon: float,
            encoder_layers: int = ITRANSFORMER_ENCODER_LAYERS,
        ) -> None:
            super().__init__()
            history_input_dim = history_steps * (local_input_channels + context_history_channels)
            future_hidden_dim = max(d_model * ITRANSFORMER_FFN_MULTIPLIER, d_model)
            self.bounded_output_epsilon = bounded_output_epsilon
            self.history_encoder = FeedForward(history_input_dim, future_hidden_dim, d_model, dropout=dropout)
            self.static_encoder = FeedForward(
                static_feature_count,
                max(d_model, static_feature_count * 2),
                d_model,
                dropout=dropout,
            )
            encoder_layer = nn.TransformerEncoderLayer(
                ...
                dim_feedforward=d_model * ITRANSFORMER_FFN_MULTIPLIER,
                dropout=dropout,
                activation="gelu",
                batch_first=True,
                norm_first=True,
            ...
            self.horizon_embedding = nn.Embedding(forecast_steps, d_model)
            self.output_grn = GatedResidualBlock(
                (d_model * 3) + context_future_channels,
                d_model * 2,
                d_model,
                dropout=dropout,
            )
        ...
        def forward(self, local_history, context_history, context_future, static_features):
            batch_size, _history_steps, node_count, _local_channels = local_history.shape
            history_context = context_history[:, :, None, :].expand(-1, -1, node_count, -1)
            history_inputs = torch.cat((local_history, history_context), dim=-1)
            history_inputs = history_inputs.permute(0, 2, 1, 3).reshape(batch_size, node_count, -1)
            history_tokens = self.history_encoder(history_inputs)
            static_tokens = self.static_encoder(static_features)
            encoded_tokens = self.encoder(history_tokens + static_tokens)
            horizon_index = torch.arange(context_future.shape[1], device=context_future.device, dtype=torch.long)
            horizon = self.horizon_embedding(horizon_index)[None, :, None, :].expand(batch_size, -1, node_count, -1)
            encoded = encoded_tokens[:, None, :, :].expand(-1, context_future.shape[1], -1, -1)
    ...
    class ITransformerNoGraph:  # pragma: no cover
        def __init__(self, *_args, **_kwargs) -> None:
            _require_torch()
...
def evaluate_itransformer_model(
    model,
    prepared_dataset: state_base.PreparedDataset,
    windows: world_model_base.FarmWindowDescriptorIndex,
    *,
    ...
    targets = np.zeros_like(predictions, dtype=np.float32)
    valid = np.zeros_like(predictions, dtype=np.float32)
    loader = _build_itransformer_dataloader(
        prepared_dataset,
        windows=windows,
        batch_size=batch_size,
        device=device,
    ...
    *,
    ...
    try:
        ...
        for epoch_index in range(start_epoch, max_epochs + 1):
            ...
            is_best_epoch = False
            if best_state is None or _selection_metric_improved(
                selection_metric,
                val_mae_pu=val_mae_pu,
                val_rmse_pu=val_rmse_pu,
                best_val_mae_pu=best_val_mae_pu,
            ...
            epoch_progress.set_postfix_str(
                " ".join(
                    [
                        ...
                        f"{_selection_metric_label(selection_metric)}="
                        f"{_selection_metric_value(selection_metric, val_mae_pu=val_mae_pu, val_rmse_pu=val_rmse_pu):.4f}",
                        "best="
                        f"{_selection_metric_value(selection_metric, val_mae_pu=best_val_mae_pu, val_rmse_pu=best_val_rmse_pu):.4f}",
                    ]
                )
            )
            if should_stop:
...
def train_itransformer_model(
    ...
    *,
    ...
    try:
        ...
        for epoch_index in range(start_epoch, max_epochs + 1):
            model.train()
            train_loader = _build_itransformer_dataloader(
                prepared_dataset,
                windows=prepared_dataset.train_windows,
                batch_size=batch_size,
                device=resolved_device,
            ...
            try:
                ...
                batch_progress.close()
            epochs_ran = epoch_index
            val_metrics = evaluate_itransformer_model(
                model,
                prepared_dataset,
                prepared_dataset.val_rolling_windows,
                batch_size=resolved_eval_batch_size,
...
def execute_itransformer_job(
    ...
    *,
    ...
    try:
        training_outcome = train_itransformer_model(
            prepared_dataset,
            selection_metric=selection_metric,
            device=resolved_device,
            seed=seed,
        ...
        evaluation_results: list[tuple[str, str, world_model_base.FarmWindowDescriptorIndex, EvaluationMetrics]] = []
        for split_name, eval_protocol, windows in iter_evaluation_specs(prepared_dataset):
            metrics = evaluate_itransformer_model(
                training_outcome.model,
                prepared_dataset,
                windows,
                batch_size=resolved_eval_batch_size,
...
def execute_training_job(
    ...
    *,
    variant_spec: ExperimentVariant | None = None,
    selection_metric: str = SELECTION_METRIC_RMSE,
    ...
    resolved_selection_metric = resolve_selection_metrics((selection_metric,))[0]
...
def _job_key(dataset_id: str, model_variant: str, selection_metric: str) -> tuple[str, str, str]:
    return dataset_id, model_variant, selection_metric


def _job_identity(*, dataset_id: str, model_variant: str, selection_metric: str, feature_protocol_id: str) -> dict[str, str]:
    return {
...
def _job_checkpoint_path(paths: ResumePaths, *, dataset_id: str, model_variant: str, selection_metric: str) -> Path:
    return paths.checkpoints_dir / f"{dataset_id}__{model_variant}__{selection_metric}.pt"
...
def sort_result_frame(frame: pl.DataFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .with_columns(
            ...
            pl.col("selection_metric")
            .replace_strict(_SELECTION_METRIC_ORDER, default=len(_SELECTION_METRIC_ORDER))
            .alias("__selection_metric_order"),
...
def sort_training_history_frame(frame: pl.DataFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .sort(["__dataset_order", "__model_variant_order", "__selection_metric_order", "epoch"])
        .drop(["__dataset_order", "__model_variant_order", "__selection_metric_order"])
    )
...
def _completed_job_keys(frame: pl.DataFrame) -> set[tuple[str, str, str]]:
    if frame.is_empty():
        return set()
    return {
        _job_key(dataset_id, model_variant, selection_metric)
        for dataset_id, model_variant, selection_metric in frame.select(
            ["dataset_id", "model_variant", "selection_metric"]
        )
        .unique()
        .iter_rows()
    }
...
def run_experiment(
    *,
    ...
    resolved_selection_metrics = resolve_selection_metrics(selection_metrics)
    ...
    total_jobs = len(resolved_dataset_ids) * len(variant_specs) * len(resolved_selection_metrics)
    ...
    try:
        for dataset_id in resolved_dataset_ids:
            for variant_spec in variant_specs:
                ...
                for selection_metric in resolved_selection_metrics:
                    current_job_key = _job_key(dataset_id, variant_spec.model_variant, selection_metric)
                    ...
                    job_progress.set_postfix_str(f"{dataset_id}/{variant_spec.model_variant}/{selection_metric}")
                    job_progress.update(1)
    finally:
        job_progress.close()
