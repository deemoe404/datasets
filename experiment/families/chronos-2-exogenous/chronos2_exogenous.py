...
def load_pipeline(*, model_id: str = MODEL_ID, device: str | None = None):
    try:
        ...
        raise ImportError(
            ...
            "Rebuild experiment/families/chronos-2-exogenous/.conda with ./create_env.sh."
        ) from exc
...
def load_covariate_series_frame(
    ...
    *,
    ...
    spec, series_path = resolve_turbine_series_path(
        dataset_id,
    ...
    return spec, series_path, covariate_columns, series
...
def _resolve_dataset_progress_plan(
    ...
    *,
    ...
    selected_turbine_ids = resolve_selected_turbine_ids(spec, turbine_ids)
    spec, series_path, target_series = load_target_series_frame(
        dataset_id,
        cache_root=cache_root,
...
def evaluate_univariate_covariate_pack(
    ...
    *,
    ...
    spec, _, covariate_columns, series = load_covariate_series_frame(
        dataset_id,
        pack=pack,
        cache_root=cache_root,
        turbine_ids=selected_turbine_ids,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional label suffix for the formal run record under experiment/artifacts/runs/chronos2_exogenous/.",
    )
    parser.add_argument(
        "--no-record-run",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    selected_stages = tuple(args.covariate_stages) if args.covariate_stages else DEFAULT_COVARIATE_STAGES
    selected_eval_protocols = tuple(args.eval_protocols) if args.eval_protocols else EVAL_PROTOCOL_CHOICES
    results = run_experiment(
        dataset_ids=selected_dataset_ids,
        covariate_stages=selected_stages,
    ...
    if not args.no_record_run:
        implementation_labels = (
            (("reference",) if args.include_power_only_reference else ())
            + selected_stages
        )
        record_cli_run(
            family_id=FAMILY_ID,
