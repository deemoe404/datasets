...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Chunk/log working directory. Defaults to experiment/families/chronos-2-exogenous/.work/full-run-<timestamp>.",
    )
    parser.add_argument(
        "--output-path",
        type=Path,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.no_record_run:
        implementation_labels = (
            ...
            + (tuple(args.covariate_stages) if args.covariate_stages else DEFAULT_COVARIATE_STAGES)
        )
        record_cli_run(
            family_id=FAMILY_ID,
            repo_root=REPO_ROOT,
            invocation_kind="full_orchestrator",
            entrypoint="experiment/families/chronos-2-exogenous/run_exogenous_full.py",
            args=vars(args),
            output_path=args.output_path or default_output_path(),
            result_row_count=result.height,
            dataset_ids=DEFAULT_DATASETS,
