...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Chunk/log working directory. Defaults to experiment/families/chronos-2/.work/full-run-<timestamp>.",
    )
    parser.add_argument(
        "--output-path",
        type=Path,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.no_record_run:
        record_cli_run(
            ...
            entrypoint="experiment/families/chronos-2/run_power_only_full.py",
            args=vars(args),
            output_path=args.output_path or default_output_path(),
            result_row_count=result.height,
            dataset_ids=DEFAULT_DATASETS,
            feature_protocol_ids=("power_only", "power_stats_history"),
