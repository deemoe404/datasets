...
_OUTPUT_PATH = _REPO_ROOT / "experiment" / "agcrn-official-aligned.csv"
...
def require_torch() -> tuple[Any, Any, Any, Any, Any]:
    if torch is None or nn is None or F is None or DataLoader is None or Dataset is None:
        raise ImportError(
            ...
            "Create experiment/agcrn/.conda with ./create_env.sh."
        )
...
def resolve_cache_paths(dataset_id: str, *, cache_root: str | Path = _CACHE_ROOT) -> TaskCachePaths:
    ...
    return TaskCachePaths(
        ...
        turbine_static_path=task_dir / "turbine_static.parquet",
    )
...
def ensure_task_cache(dataset_id: str, *, cache_root: str | Path = _CACHE_ROOT) -> TaskCachePaths:
    ...
    _ensure_repo_src_on_path()
    try:
        from wind_datasets import build_gold_base, build_task_cache
    ...
    try:
        ...
        build_task_cache(dataset_id, build_task_spec(), cache_root=cache_root, quality_profile=QUALITY_PROFILE)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Output CSV path. Defaults to experiment/agcrn-official-aligned.csv in the repo root.",
    )
    parser.add_argument(
        "--max-train-origins",
        type=int,
