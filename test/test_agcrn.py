...
def test_prepare_dataset_builds_multichannel_source_tensor_for_power_wd_yaw_hist_sincos(tmp_path, monkeypatch) -> None:
    module = _load_module()
    cache_root = tmp_path / "cache"
    _build_temp_cache(
        cache_root,
    ...
    _patch_temp_bundle_loader(monkeypatch, module, cache_root, feature_protocol_id="power_wd_yaw_hist_sincos")

    prepared = module.prepare_dataset(
        "toy_dataset",
        variant_spec=module.resolve_variant_specs((module.POWER_WD_YAW_HIST_SINCOS_MODEL_VARIANT,))[0],
        cache_root=cache_root,
    )

    assert prepared.model_variant == module.POWER_WD_YAW_HIST_SINCOS_MODEL_VARIANT
    assert prepared.feature_protocol_id == module.POWER_WD_YAW_HIST_SINCOS_FEATURE_PROTOCOL_ID
...
def test_prepare_dataset_builds_multichannel_source_tensor_for_power_wd_yaw_lrpm_hist_sincos(
    ...
    _patch_temp_bundle_loader(monkeypatch, module, cache_root, feature_protocol_id="power_wd_yaw_lrpm_hist_sincos")

    prepared = module.prepare_dataset(
        "toy_dataset",
        variant_spec=module.resolve_variant_specs((module.POWER_WD_YAW_LRPM_HIST_SINCOS_MODEL_VARIANT,))[0],
        cache_root=cache_root,
    )

    assert prepared.model_variant == module.POWER_WD_YAW_LRPM_HIST_SINCOS_MODEL_VARIANT
    assert prepared.feature_protocol_id == module.POWER_WD_YAW_LRPM_HIST_SINCOS_FEATURE_PROTOCOL_ID
    assert prepared.input_channel_names == (
        "target_pu",
        "wind_direction_sin",
        "wind_direction_cos",
