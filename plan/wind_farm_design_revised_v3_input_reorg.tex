...
\]
处截断。若某节点在时刻 $t$ 的全部本地观测通道都不可用，则该时刻跳过本机 posterior update，直接保留 prior state；全局池化则在有可用节点时只对可用节点求平均，若所有节点都不可用则退回到简单平均。
...
\[
r_{i,t}=\mathrm{Enc}_{obs}\left([\bar o^{enc}_{i,t},\, s_i]\right).
\]

其中：
\[
\bar o^{enc}_{i,t}=\left[o_{i,t},\, q^{obs}_{i,t},\, \Delta t^{obs}_{i,t},\, a^{audit}_{i,t}\right],
\]
...
其中：
\[
v^{src}_{j,t}=\mathrm{Enc}^{upd}_{src}\left([\bar o^{enc}_{j,t},\, z^-_{j,t},\, h^-_{j,t},\, s_j]\right).
\]
...
\]
\[
d^{\parallel}_{ij,t}=\Delta x_{ij} C_t+\Delta y_{ij} S_t,
\qquad
d^{\perp}_{ij,t}=-\Delta x_{ij} S_t+\Delta y_{ij} C_t.
\]
\[
...
历史窗口中的状态闭环按 ``\textbf{先 Transition 得到 prior，再用真实观测做 Update}'' 执行。当前实现在第一个历史步 $t_0$ 上直接从初始化状态做 correction；对后续历史步 $t>t_0$，共享 rollout 阶段的 Transition / Transition$_g$ 参数完成 prior propagation，而不是额外再写一套 history-only dynamics。

给定上一时刻后验
\[
\left\{z^+_{i,t-1},\, h^+_{i,t-1}\right\}_{i=1}^N,\qquad g^+_{t-1},
\]
先由上一时刻全局状态解码当前时刻的先验站点级摘要
\[
\hat c^{met}_{t|t-1}=\mathrm{Dec}^{met}_g\left(g^+_{t-1},\, \mathrm{cal}_{t}\right),\qquad
\hat c_{t|t-1}=\left[\hat c^{met}_{t|t-1},\, \mathrm{cal}_{t}\right],
\]
再由上一时刻节点状态构造先验源机组摘要
\[
\hat\eta^{src}_{j,t|t-1}=\mathrm{Enc}^{pred}_{src}\left([z^+_{j,t-1},\, h^+_{j,t-1},\, g^+_{t-1},\, s_j]\right).
\]
于是历史 prior propagation 阶段的动态图特征写为
\[
\hat p^{dyn,pred}_{i\leftarrow j,t|t-1}=\left[\hat d^{\parallel}_{ij,t|t-1},\, \hat d^{\perp}_{ij,t|t-1},\, \hat\psi^{wake}_{ij,t|t-1}\right],
\]
并由
\[
e^{pred}_{i\leftarrow j,t|t-1}=\mathrm{MLP}_e^{pred}\left([s_i,\, s_j,\, p^{stat}_{i\leftarrow j},\, \hat p^{dyn,pred}_{i\leftarrow j,t|t-1},\, \hat\eta^{src}_{j,t|t-1},\, g^+_{t-1},\, \hat c_{t|t-1}]\right),
\]
\[
a^{pred}_{i\leftarrow j,t|t-1}=\sigma\left(\mathrm{MLP}_a^{pred}\left([s_i,\, s_j,\, p^{stat}_{i\leftarrow j},\, \hat p^{dyn,pred}_{i\leftarrow j,t|t-1},\, \hat\eta^{src}_{j,t|t-1},\, g^+_{t-1},\, \hat c_{t|t-1}]\right)\right),
\]
\[
m^{pred}_{i,t|t-1}=\frac{\sum_{j\neq i} a^{pred}_{i\leftarrow j,t|t-1}e^{pred}_{i\leftarrow j,t|t-1}}{\epsilon+\sum_{j\neq i} a^{pred}_{i\leftarrow j,t|t-1}}
\]
得到先验图消息。随后用共享的 Transition 接口完成一步 prior propagation：
\[
z^-_{i,t}=\mathrm{GRU}^{tr}_z\left([m^{pred}_{i,t|t-1},\, g^+_{t-1},\, \hat c^{met}_{t|t-1},\, \mathrm{cal}_t,\, h^+_{i,t-1},\, s_i],\, z^+_{i,t-1}\right),
\]
\[
h^-_{i,t}=\mathrm{GRU}^{tr}_h\left([z^-_{i,t},\, m^{pred}_{i,t|t-1},\, g^+_{t-1},\, \hat c^{met}_{t|t-1},\, \mathrm{cal}_t,\, s_i],\, h^+_{i,t-1}\right).
\]
\noindent 同时，全局 prior state 记为 $g_t^-$，其更新方式见后文的 Update$_g$ / Transition$_g$ 小节。

最后再用真实观测执行 posterior correction。先由 prior state 解码当前观测：
\[
...
\]
于是局部 posterior update 写为
\[
z^+_{i,t}=\mathrm{GRU}^{upd}_z\left([\nu_{i,t},\, m^{upd}_{i,t},\, g_t^-,\, c_t,\, s_i],\, z^-_{i,t}\right),
\]
\[
h^+_{i,t}=\mathrm{GRU}^{upd}_h\left([z^+_{i,t},\, \nu_{i,t},\, m^{upd}_{i,t},\, g_t^-,\, c_t,\, s_i],\, h^-_{i,t}\right).
\]
\noindent 若该时刻节点 $i$ 没有任何可用本地观测，则保持 $z^+_{i,t}=z^-_{i,t}$、$h^+_{i,t}=h^-_{i,t}$。

\subsubsection{rollout 阶段的 Transition 与内部预测量}
未来窗不再接入真实观测，只保留 open-loop transition。对每个 horizon 步 $\tau=1,\ldots,H$，先由上一时刻全局状态解码未来站点级摘要：
\[
...
\]
再由上一时刻节点状态构造供边网络使用的 future source summary：
\[
\hat\eta^{src}_{j,t+\tau}=\mathrm{Enc}^{pred}_{src}\left([z_{j,t+\tau-1|t},\, h_{j,t+\tau-1|t},\, g_{t+\tau-1|t},\, s_j]\right).
\]
于是 rollout 阶段的动态图特征写为
\[
\hat p^{dyn,pred}_{i\leftarrow j,t+\tau}=\left[\hat d^{\parallel}_{ij,t+\tau},\, \hat d^{\perp}_{ij,t+\tau},\, \hat\psi^{wake}_{ij,t+\tau}\right],
\]
并由
\[
e^{pred}_{i\leftarrow j,t+\tau}=\mathrm{MLP}_e^{pred}\left([s_i,\, s_j,\, p^{stat}_{i\leftarrow j},\, \hat p^{dyn,pred}_{i\leftarrow j,t+\tau},\, \hat\eta^{src}_{j,t+\tau},\, g_{t+\tau-1|t},\, \hat c_{t+\tau}]\right),
\]
\[
a^{pred}_{i\leftarrow j,t+\tau}=\sigma\left(\mathrm{MLP}_a^{pred}\left([s_i,\, s_j,\, p^{stat}_{i\leftarrow j},\, \hat p^{dyn,pred}_{i\leftarrow j,t+\tau},\, \hat\eta^{src}_{j,t+\tau},\, g_{t+\tau-1|t},\, \hat c_{t+\tau}]\right)\right),
\]
\[
m^{pred}_{i,t+\tau}=\frac{\sum_{j\neq i} a^{pred}_{i\leftarrow j,t+\tau}e^{pred}_{i\leftarrow j,t+\tau}}{\epsilon+\sum_{j\neq i} a^{pred}_{i\leftarrow j,t+\tau}}
\]
得到未来图消息。

随后用 open-loop Transition 继续推进节点状态：
\[
z_{i,t+\tau|t}=\mathrm{GRU}^{tr}_z\left([m^{pred}_{i,t+\tau},\, g_{t+\tau-1|t},\, \hat c^{met}_{t+\tau},\, \mathrm{cal}_{t+\tau},\, h_{i,t+\tau-1|t},\, s_i],\, z_{i,t+\tau-1|t}\right),
\]
\[
h_{i,t+\tau|t}=\mathrm{GRU}^{tr}_h\left([z_{i,t+\tau|t},\, m^{pred}_{i,t+\tau},\, g_{t+\tau-1|t},\, \hat c^{met}_{t+\tau},\, \mathrm{cal}_{t+\tau},\, s_i],\, h_{i,t+\tau-1|t}\right).
\]

\noindent 语义上，$\hat c^{met}_{t+\tau}$ 是未来站点级摘要的内部预测量。若训练阶段可以从真实未来观测构造站点级统计量，则定义
\[
c^{met,*}_{t+\tau}=\left[\widetilde{\bar{ws}}_{t+\tau},\, C_{t+\tau},\, S_{t+\tau},\, R_{t+\tau},\, \rho^{ws}_{t+\tau},\, \rho^{wd}_{t+\tau}\right]
\]
并用其作为 $\hat c^{met}_{t+\tau}$ 的弱监督目标；$\hat\eta^{src}_{j,t+\tau}$ 则仅作为 latent source summary，通过 forecast / graph losses 间接训练。

\subsubsection{全局状态 Update\_g / Transition\_g}
历史窗的 posterior correction 之后，当前实现先对节点后验状态做带可用性掩码的平均池化，再更新全局状态。记
\[
\rho^{node}_{i,t}=
\begin{cases}
...
\]
\noindent 若当前时刻所有节点都没有可用本地观测，则退回到简单平均池化。

在历史窗 prior propagation 和未来 rollout 阶段，全局转移共用同一接口。记
\[
\bar n^{tr}_{t+\tau}=\frac{1}{N}\sum_{i=1}^{N}[z_{i,t+\tau|t},\, h_{i,t+\tau|t}],
\]
则
\[
g_{t+\tau|t}=\mathrm{GRU}^{tr}_g\left([\bar n^{tr}_{t+\tau},\, \hat c^{met}_{t+\tau},\, \mathrm{cal}_{t+\tau}],\, g_{t+\tau-1|t}\right).
\]
\noindent 因而历史窗 prior propagation 与未来窗 rollout 在 local/global transition 上使用的是同一套状态机。
