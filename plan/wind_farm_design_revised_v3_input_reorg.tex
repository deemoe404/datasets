...
s^{site}_t=
\left[
  \widetilde{ws}^{mean}_t,\,
  \widetilde{wd}^{\cos,mean}_t,\,
  \widetilde{wd}^{\sin,mean}_t,\,
  \widetilde{r}^{wd}_t,\,
  \rho^{ws}_t,\,
  \rho^{wd}_t
\right],
...
其中：$\widetilde{r}^{wd}_t$ 表示风向圆均值的 resultant length，
$\rho^{ws}_t,\rho^{wd}_t$ 分别表示风速与风向的可用率。
