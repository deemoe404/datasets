...
\paragraph{global + local decoder follow-ups（2026-04-22 代码版；2026-04-23 首轮实验）}
在当前 trunk 上，后续 decoder-side 结构探索先收敛成两个不引入 cluster state 的 whole-farm 版本。第一条是 \texttt{global\_local\_residual}：把 anchor-relative 预测拆成一个只依赖 \(g_{t+\tau|t}\) 与 \(\mathrm{emb}(\tau)\) 的全场共享 residual，再加一个只依赖 \([h_{i,t+\tau|t}, z_{i,t+\tau|t}, s_i, \mathrm{emb}(\tau)]\) 的 local correction；local 分支在每个 horizon 上都做按 turbine weight 的零均值约束，从而避免 global/local 学同一件事。第二条是 \texttt{global\_local\_increment}：保持同样的 global/local 分解和 zero-mean 约束，但把输出语义改成 per-step increment，并从 persistence anchor 逐步累积回 level。对应的 Ubuntu 首轮最小 ablation（\texttt{global\_local\_ablation\_20260422\_farm0p00\_val\_rmse}，固定 \texttt{farm\_loss\_weight=0.0}、\texttt{ramp\_loss\_weight=0.0}、\texttt{selection\_metric=val\_rmse\_pu}、\texttt{batch\_size=432}）现已完成：\texttt{global\_local\_residual} 的 val/test rolling RMSE 为 \texttt{0.161666 / 0.169643}，test non-overlap RMSE 为 \texttt{0.168121}；\texttt{global\_local\_increment} 则为 \texttt{0.162425 / 0.178203}，test non-overlap RMSE 为 \texttt{0.176716}。二者都没有超过当前 residual trunk 最优的 \texttt{ramp0p05}（test rolling/non-overlap RMSE \texttt{0.168230 / 0.166406}），其中 increment 语义明显更差。因此当前更稳的结论是：whole-farm global + zero-mean local 这条 decoder decomposition 本身并未自动带来收益，\texttt{global\_local\_residual} 可以保留为 clean 备选，但 working head 仍应维持在 \texttt{residual\_persistence}；\texttt{global\_local\_increment} 暂不值得继续追加主线预算。

\paragraph{residual edge-gating 2x2（2026-04-22 本地代码版）}
在当前 \texttt{residual\_persistence + farm\_loss\_weight=0.0 + ramp\_loss\_weight=0.0} trunk 上，edge aggregation 与 wake 单位解释的最小对照先固定为 residual-only 的显式 variants，而不是进一步做成 family-wide 可组合 CLI 轴。控制组仍是当前 \texttt{residual\_persistence}。其余三条本地代码线分别是：\texttt{residual\_persistence\_gated\_sum}，只把图聚合从
\[
m_i=\frac{\sum_j g_{ij}\,\phi_{ij}}{\sum_j g_{ij}+\varepsilon}
\]
改成
\[
m_i=\sum_j g_{ij}\,\phi_{ij};
\]
\texttt{residual\_persistence\_rotor\_units\_wake}，只把 wake 内部几何解释改成先用
\[
\text{meters\_per\_rotor\_diameter}_{ij}
=
\frac{\sqrt{(\Delta x_{ij})^2 + (\Delta y_{ij})^2}}{d^{(RD)}_{ij}}
\]
把 raw \(\Delta x,\Delta y\) 转成 rotor-diameter units，再计算 \(d^{\parallel}, d^{\perp}\)；以及同时启用这两项改动的 \texttt{residual\_persistence\_gated\_sum\_rotor\_units\_wake}。此外，checkpoint scratch eval 新增可选 \texttt{--include-edge-diagnostics}，用于输出 \texttt{transition/update} 两个 phase 上的 \(d^{\parallel}\)、\(d^{\perp}\)、wake gate、learned gate、\(\sum_j g_{ij}\) 与聚合前后 message norm 汇总；这一步只为后续有算力时的对照做观测准备，不改变默认训练路径或 formal schema。
